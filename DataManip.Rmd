...
```{r Interaction Terms}
# working model is housing.lmer.fix2

# maybe PctWhite and PctBlack interact?
housing.lmer.int1 <- lmer(log(price) ~ scale(sqfeet) + beds + baths + pets_allowed + smoking_allowed + scale(MedianIncome) + Governor + PopDensity + TaxRate + PctWhite*PctBlack + (1|state), data = data2, REML = FALSE)
summary(housing.lmer.int1)

# alt - null
D <- 2*(logLik(housing.lmer.fix2)-logLik(housing.lmer.int1))
1-pchisq(D, 1) # p-val is 1, prefer housing.lmer.int1

# maybe beds and baths interact?
housing.lmer.int2 <- lmer(log(price) ~ scale(sqfeet) + beds*baths + pets_allowed + smoking_allowed + scale(MedianIncome) + Governor + PopDensity + TaxRate + PctWhite*PctBlack + (1|state), data = data2, REML = FALSE)
summary(housing.lmer.int2)

# alt - null
D <- 2*(logLik(housing.lmer.int1)-logLik(housing.lmer.int2))
1-pchisq(D, 1) # p-val is 1, prefer housing.lmer.int2
```
