...
housing.lmer4.1 <- lmer(log(price) ~ 1 + (1|state), data = housing_subset2)
summary(housing.lmer)

housing.lmer4.2 <- lmer(log(price) ~ (1|state) + (sqfeet + beds + baths|state), data = housing_subset2)
...
hist(Dsim)
abline(v = D)
mean(Dsim >= D) # p-value is 1
```

```{r Fixed Effects}
housing.lmer.fix1 <- lmer(log(price) ~ sqfeet + beds + baths + pets_allowed + smoking_allowed + wheelchair_access + comes_furnished + scale(MedianIncome) + Governor + PopDensity + TaxRate + DeathPenalty + PctWhite + PctBlack + (1|state), data = housing_subset2)
summary(housing.lmer.fix1)

# throw out wheelchair acess and death penalty
housing.lmer.fix2 <- lmer(log(price) ~ sqfeet + beds + baths + pets_allowed + smoking_allowed +  comes_furnished + scale(MedianIncome) + Governor + PopDensity + TaxRate + PctWhite + PctBlack + (1|state), data = housing_subset2)
