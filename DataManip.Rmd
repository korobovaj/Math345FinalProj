...
```{r}
housing_subset2 <- housing_subset %>%
  filter(price > 5)
...
housing_subset2 <- housing_subset2 %>%
...
```{r}
housing.glmer <- lmer(log(price) ~ 1 + (1|state), data = housing_subset2)
summary(housing.glmer)

housing.glmer2 <- lmer(log(price) ~ sqfeet + beds + baths + pets_allowed + smoking_allowed + wheelchair_access + comes_furnished + scale(MedianIncome) + Governor + PopDensity + TaxRate + DeathPenalty + PctWhite + PctBlack + (1|state), data = housing_subset2)

housing.glmer3 <- lmer(log(price) ~ sqfeet + beds + baths + pets_allowed + smoking_allowed + wheelchair_access + comes_furnished + scale(MedianIncome) + Governor + PopDensity + TaxRate + DeathPenalty + PctWhite + PctBlack + (sqfeet + beds + baths + pets_allowed + smoking_allowed + wheelchair_access + comes_furnished|state), data = housing_subset2)

D <- 2*(logLik(housing.lmer2)-logLik(housing.lmer3))
N <- 500
Dsim <- numeric(N)
nullY <- simulate(housing.lmer3, nsim = N)
for (i in 1:N) {
  null.lmer <- refit(housing.lmer2, nullY[,i])
  alt.lmer <- refit(housing.lmer3, nullY[,i])
