...
housing.lmer.fix4.2 <- update(housing.lmer.fix4, subset = -c(79, 1579, 1770))
...
# have to make data the same length
...
```{r Refit Two}
# removing two other observations with high cook's distance values
housing.lmer.fix4.3 <- update(housing.lmer.fix4, subset = -c(79, 1579, 1770, 1229, 1231))
summary(housing.lmer.fix4.3)
```

```{r Diagnostics Final}
cd <- cooks.distance(housing.lmer.fix4.3)
plot(cd, type = "h") # one super high cook's D, let's remove it
